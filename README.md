...
One policy, one workflow, four agents. EyrAgents is a personal harness for [Claude Code](https://code.claude.com/docs/en/overview), [Codex](https://learn.chatgpt.com/codex), [OpenCode](https://opencode.ai/docs), and [Hermes Agent](https://hermes-agent.nousresearch.com/docs/). They share guidance and the commit/publication workflow on an [Omarchy](https://omarchy.org) desktop and a WSL Arch machine, with tool-specific access controls and documented enforcement limits. It is a [GNU Stow](https://www.gnu.org/software/stow/) repository: each package mirrors `$HOME`, and `make stow` links the configuration into place.
...
- **Skills in the open format.** The workflows are [Agent Skills](https://agentskills.io) `SKILL.md` files with their executables in `scripts/`, written once under `~/.agents`; Codex and OpenCode read them there, Claude Code through symlinks, and Hermes through its configured external skill library.
...
- **Deployment you can verify.** `make stow` deploys, `make verify` proves that every link resolves and that the host Codex and Hermes configs carry their managed template boundaries, and CI runs the repository checks on every push.
...
| [Hermes Agent](https://hermes-agent.nousresearch.com/docs/) | [github.com/NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) | Host-local `~/.hermes/config.yaml`, shared skills through `skills.external_dirs`, and `~/.hermes/plugins/eyragents/`; shared guidance is composed into the stable `agent.environment_hint`, preserving Hermes's own `SOUL.md` |

The tools themselves are installed outside this repository, through [mise](https://mise.jdx.dev) on both machines: Omarchy installs its own wrappers, and EyrWSL stows its wrappers. Hermes uses the pipx backend with a private Python 3.13 runtime and uv; its installer differs from the other three. Follow Omarchy's PyPI release channel, currently Hermes 0.19.0, rather than replacing the installation with a newer Git checkout.
...
[`references.txt`](references.txt) declares four official references under `~/Projects/quarry`: `claude-code` for public release/plugin/support material, and `codex`, `opencode`, and `hermes-agent` for client source and tests. Claude Code's public repository does not contain its proprietary CLI implementation. Each tool still needs official documentation, release notes and scoped runtime evidence. Hermes's installed PyPI package is the compatibility baseline when its version differs from newer GitHub releases; missing reference clones require the normal approved bootstrap.
...
[`AGENTS.md`](AGENTS.md) holds the repository invariants. The skills hold workflow procedure and the scripts hold the steps. [`docs/access.md`](docs/access.md) compares all four tools' read/write access, tool-call controls, rationale, implementation, and official sources. [`docs/design.md`](docs/design.md) gives the reasons behind the shape. [`docs/maintenance.md`](docs/maintenance.md) holds active limitations, open decisions, deferred work, and revalidation triggers.
...
├── hermes/.hermes/plugins/eyragents/      # Hermes plugin.yaml and native-tool guard adapter
├── templates/hermes/config.yaml          # managed Hermes leaves, reconciled into private host state
├── scripts/reconcile-hermes-config.py    # preserves host settings; composes shared guidance
...
├── docs/access.md                        # four-tool access matrices, decisions, implementation, and sources
...
Start with the [four-tool access matrices](docs/access.md): separate read/write rows for workspace, home, system and temporary directories, followed by tool calls, consequential actions, and parity decisions. They distinguish authorization, configured capability, and verified behavior rather than treating a location grant as read-only or a classifier decision as a human prompt.
...
On Omarchy, install the CLI with `omarchy install hermes cli --now`. On WSL, use EyrWSL's `~/.local/bin/hermes --version` wrapper. Then run `hermes model` yourself to authenticate with **ChatGPT or Codex Subscription**. Select Astra if the account catalog offers it. The v0.19 client accepts its model ID and preserves `xhigh` on the native Responses path; those facts do not establish account entitlement. Authentication and a successful live reply remain separate from installation and repository checks.
...
```text
~/.hermes/
├── config.yaml                 # private app-managed config, reconciled from the template
├── SOUL.md                     # Hermes-owned identity, preserved
├── plugins/eyragents/           # Stow-linked manifest and policy adapter
├── memories/{MEMORY,USER}.md    # Hermes-local learning, generated at runtime
├── skills/                     # Hermes-local skills and upstream-managed skill links
├── profiles/                   # optional independent profiles, not deployed by this harness
├── sessions/, state.db, logs/   # private runtime history/state
├── cron/                       # runtime schedules, created only on H's instruction
└── auth.json, .env              # authentication/secret state, never tracked or agent-inspected
```
...
The EyrAgents plugin checks native local file operands, protects credential-shaped paths and Git internals, normalizes task-relative file paths, extends native search-result filtering, and dispatches terminal commands to the installed commit gate. It also protects shared skills from `skill_manage` mutation. Python, shell subprocesses, interactive process input, remote backends, arbitrary extensions and plugin loading have the limits in [Access Policy](docs/access.md#hermes-agent). No Hermes read-only auditor or new cross-vendor reviewer implementation is claimed; the shared `spar-claude` bridge is the Astra session's review route.
...
- `eyrsync`, this repository's own skill, starts with reference/evidence coverage for all four tools, then reconciles the harness against current official documentation, releases, available version-matched source and runtime evidence. Its access pass connects matrix cells to decisions and implementation; its sibling pass checks shared contracts. Run it for relevant interface changes, any managed tool's breaking release, missing references or conflicting evidence, or periodically. The Agent Skills specification remains the shared format reference.
...
Receipts are immutable while needed, not permanent archives. At completion, the commit skill's [Receipt Close-Out](agents/.agents/skills/commit/SKILL.md#receipt-close-out) uses `commit-candidate --close ID... --dry-run` with the same record root, then closes that exact selection. Default eligibility covers verified publications, simple rejections, and committed candidates matching their immutable receipts and covered by selected verified publication ancestry. Active, ambiguous, corrupt and foreign-worktree records remain. H can explicitly accept `--accept-unverified` close-out after the unknown outcome and unresolved issue have a canonical disposition; it never claims a successful push or verification. Exact-ID transient markers support interrupted cleanup. Stable lockfiles remain; historical/unknown records need scoped review, not a directory or age sweep.
...
`review-brief` records full HEAD, read-only source/index fingerprints, raw indexed byte/mode agreement and actual gate commands/results. It batches requested Make goals once, preserving argument order and reporting the combined exit status, not invented per-target results; Makefile dependencies determine recipe order. Nonempty inherited `MAKEFLAGS`, `MFLAGS`, `GNUMAKEFLAGS` or `MAKEFILES` refuse gate execution, never silently turn a dry-run into execution. Worktree diffs refuse configured clean/process filters, even unused ones, and a present `GIT_CONFIG` override that could hide them from the query; staged/range diffs remain available. Raw matching ignores filters/stat shortcuts, and inaccessible paths never establish deletion. Direct internal tracked regular-file symlink targets may be attested; unsupported links and other unknown inputs remain explicit.

Failed gates or source drift keep a scanned diagnostic artifact and return exit 1. Plan mode reads no source contents, runs no gates and writes only in private scratch or to a new, ignored, untracked Markdown file directly under `.eyr-plans/<workstream>/{audit,spar}/`; existing workstream/artifact directories must be private, owned and unlinked, and `governance` is reserved. The generator creates no directories or ignore rules and never overwrites an artifact. Fingerprints are not Git tree IDs or runtime/host attestations. Same-workstream reuse requires the original passing evidence plus unchanged relevant contents/context; untracked inputs, unattested links, unknown context, or a fresh `--no-gates` artifact cannot establish it. No persistent cache is implied.

Substantial work uses a lean `.eyr-plans/<workstream>/checkpoint.md`, read on resume, compaction and session handoff. Its remaining atomic change map assigns behavior, code/tests/docs, dependencies and ownership before implementation or delegation. The primary updates current state and links needed `audit/` or `spar/` evidence rather than copying it into a journal. The canonical [Workstream Checkpoints](agents/.agents/shared-guidance.md#workstream-checkpoints) rule needs no extra skill: automatically remove workflow-owned artifacts once no implementation, review, approval or concrete handoff needs them, then remove empty directories. A generic deferred issue belongs in canonical docs, not a completed workstream archive. Account for all retained items and their next use, not just an empty `/tmp`. Preserve user files; approval/publication records follow the separate close-out procedure.
